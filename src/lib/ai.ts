...
ZORUNLU KURALLAR:
1. Bir içerik AYNI ANDA BİRDEN FAZLA platformda olabilir — hepsini yaz!
2. Sadece %100 emin olduğun platformları yaz. Şüphe varsa → []
3. HBO Max Türkiye'de YOK. HBO içerikleri → TOD
4. Hulu, Peacock, Paramount+ Türkiye'de YOK
5. Türkçe özet: 2 cümle, spoiler yok

ÖRNEK (birden fazla platform):
- Inception → ["Netflix", "Amazon Prime"] (hem Netflix hem Prime'da var)
- Oppenheimer → ["TOD", "Amazon Prime"] (hem TOD hem Prime'da var)

İÇERİKLER:
${list}

SADECE JSON (başka hiçbir şey yazma):
[{"id": <number>, "platforms": ["Netflix", "Amazon Prime"], "overview_tr": "Türkçe özet."}]`
